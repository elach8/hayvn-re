...
const PORTAL_LINKS = [
  ...
  { href: '/portal/tours', label: 'Tours' },       // future
  { href: '/portal/offers', label: 'Offers' },     // future
  { href: '/portal/messages', label: 'Messages' }, // future
];
          ...
          <div className="mx-auto max-w-5xl px-4 py-2 flex flex-wrap items-center gap-2 text-xs">
            {PORTAL_LINKS.map((link, idx) => (
              <Link
                key={link.href}
                href={link.href}
                className={`rounded-full px-3 py-1 border border-gray-200 hover:bg-gray-50 ${
                  idx === 0 ? 'bg-gray-900 text-white hover:bg-gray-900' : 'text-gray-700'
                }`}
              >
                {link.label}
              </Link>
            ...
            {error.toLowerCase().includes('sign in') && (
              <div className="mt-2">
                <button
                  type="button"
                  onClick={() => router.push('/portal/sign-in')}
                  className="rounded-md bg-red-600 text-white text-xs font-medium px-3 py-1.5 hover:bg-red-700"
                >
                  Go to sign in
                </button>
              </div>
          ...
          <div className="space-y-3">
            {journeys.map((j) => {
              ...
              return (
                <article
                  key={j.id}
                  className="rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm flex flex-col gap-2"
                >
                  ...
                  <footer className="pt-2 border-t border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 text-xs">
                    <p className="text-gray-500">
                      Properties you&apos;re viewing, tours, and offers will
                      appear in your journey details.
                    </p>
                    <div className="flex gap-2 justify-end">
                      <Link
                        href="/portal/properties"
                        className="rounded-lg border border-gray-200 px-3 py-1.5 text-[11px] text-gray-600 hover:bg-gray-50"
                      >
                        View properties
                      </Link>
                      {/* Future: a dedicated journey page */}
                      {/* <Link
